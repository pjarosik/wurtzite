...
def interpolate_path(path, n_points, xp=np):
    """
    ...
    if isinstance(path, np.ndarray):
        path_to_verify = path.flatten().tolist()

    if not path_to_verify or n_points < 2:
        return path

    pts = xp.asarray(path)
    # długości odcinków
    seg_vecs = pts[1:] - pts[:-1]
    seg_lengths = xp.linalg.norm(seg_vecs, axis=1)
    total_length = xp.sum(seg_lengths)

    if total_length == 0:
        return [tuple(pts[0])] * n_points

    # pozycje docelowe wzdłuż ścieżki
    target_dists = xp.linspace(0, total_length, n_points)

    result = []
    seg_cum = xp.cumsum(seg_lengths)
    seg_start = xp.insert(seg_cum[:-1], 0, 0.0)

    for d in target_dists:
        idx = xp.searchsorted(seg_cum, d)
