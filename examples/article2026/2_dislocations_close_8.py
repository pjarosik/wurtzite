"""
The two-dislocation figure of 2_dislocations_8.py, with d_1 moved CLOSE to d_2:
about two unit cells apart along OX instead of seven.

Everything else -- the Burgers vectors, the core types, the integration paths,
the glide plane -- is as in 2_dislocations_8.py; only `d1_cells_towards_d2`
below differs. Keeping the two scripts side by side makes the pair of figures
comparable: the same construction at two separations.
"""
import dataclasses
import pickle
...
# that bounding box [A].
fov_margin = 8.0
# Margin on the LEFT [A]. Kept separate, and wider, because that is where the
# cut of d_2 runs out of the figure: the two lips of the cut have slipped past
# each other there, and how the lattice closes up again is worth seeing. Still
# clipped to the extent of the crystal, so no vacuum is shown.
fov_margin_left = 20.0
...
# of d_2 stopped at x = -3.4 A, 3.0 A short of the left edge of the crystal at
# that height, and both the drawn line and the tear in the lattice ended there
# in mid-air.
#
...
# If True, the panel carrying the paths also shows the glide plane AS THE
# CALCULATION USES IT, rather than as the harpoons of the other panel show it.
#
# The two are the same curve, but not the same array. `find_glide_plane` traces
# the cut in the configuration the dislocation is inserted INTO, and that traced
# polyline is what `get_integration_path_via_energy` is handed: it decides from
# it on which side of the cut the atom lies, and `filter_lattice_by_curve` then
# cuts the lattice graph along it, so that the Dijkstra path cannot cross. That
# is what selects the side an atom is displaced from. The harpoons in the other
# panel are that same polyline pushed into the configuration AFTER the insertion
# by `displace_glide_plane`, which is display only -- it averages the
# displacements of the atoms on either side, and near the core it smooths.
#
# Drawn here as a plain line: it is the bare curve the code works with, and the
# harpoon decoration would claim more than that -- it says which way each lip
# slips, which this polyline does not carry.
#
# NOTE: this is the barrier for the ATOM paths only. The paths that end in a
# dislocation core (`path_show_dislocation_paths`) go through
# `get_integration_path`, which is called with glide_plane=None and falls back
# to a straight horizontal line through the core of the inserted dislocation,
# in its own local frame. That cruder barrier is not drawn.
path_show_computed_cut = True
path_cut_linewidth = 1.6
path_cut_linestyle = "solid"
...
# How far d_1 is moved TOWARDS d_2, in whole lattice translations a_1 = [1,0,0]
# (Miller). a_1 runs along OX and is the period of the lattice in that
# direction, so a whole number of them leaves d_1 on an equivalent site of the
# crystal and its core structure -- the 8-ring, see `d1_core_offset` -- is the
# one it would have had anyway. A shift that is not a lattice translation would
# have to be compensated there instead.
#
# In 2_dislocations_8.py the two cores stand 22.31 A = 7.02 a apart along OX;
# five cells closer leaves 6.41 A = 2.02 a, which is the separation this script
# is for. Note that the cores are also 2.41 A apart along OY, so the distance
# between them is 6.85 A rather than 6.41 A.
d1_cells_towards_d2 = 5
towards_d2 = d1_cells_towards_d2 * np.asarray(
    l0.cell.to_cartesian_indices(np.asarray([1, 0, 0])), dtype=float)
...
filename_prefix = f"2_dislocations_close_{timestamp}"
...
dislocations = [
    wzt.model.DislocationDef(
        ...
        # moved one unit cell up, on the site that gives an 8-ring core, and
        # then `d1_cells_towards_d2` cells along +OX, towards d_2.
...
# See `_corners` above: far enough that the cut leaves the crystal whichever
# way it runs from whichever core, so that it never ends inside the lattice.
...
def field_of_view(extra=()):
    ...
    xlim = (max(points[:, 0].min() - fov_margin_left, atoms[:, 0].min()),
...
def main(params):
    ...
    # The glide plane as the calculation uses it, i.e. the traced polyline in
    # the configuration the paths live in -- NOT carried into the deformed one.
    drawn_cut = None
    ...
    for i in range(n_ready_dislocations, len(dislocations)):
        ...
        if paths and i + 1 == path_dislocation_nr:
            ...
            # The very polyline handed to get_integration_path_via_energy: the
            # cut of the dislocation being inserted, untouched by
            # displace_glide_plane.
            drawn_cut = log.last_glide_planes[-1]
    ...
    for i, (l, ds, path, plane) in enumerate(
        ...
        if path is not None:
            ...
            if path_show_computed_cut and drawn_cut is not None:
                # core_radius = 0: the whole curve is shown, because the whole
                # of it is what cuts the lattice graph.
                for segment in clip_glide_plane(drawn_cut, l.coordinates,
                                                [dd.position for dd in ds],
                                                core_radius=0.0,
                                                bbox=(xlim, ylim)):
                    ax.plot(segment[:, 0], segment[:, 1],
                            color=drawn_path_dislocation.color,
                            linewidth=path_cut_linewidth,
                            linestyle=path_cut_linestyle, zorder=8500)
    ...
    if is_save_state:
        with open(f"state_{timestamp}.pkl", "wb") as f:
            state = {
                ...
                # The glide plane as the calculation uses it (see
                # `path_show_computed_cut`), in global coordinates.
                "computed_cut": drawn_cut,
