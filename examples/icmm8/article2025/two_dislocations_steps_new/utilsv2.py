...
# TODO pozostalo poprawic jacobian_points (dodac czlon dot. - beta d2 (xs) * grad xs)
# xs -- dokladnie punkt wyznaczony z get_d0
# grad xs
...
def get_f(crystal_plane, point, crystal, dislocation, debug=False):
    # Crystal plane zaczepiona powinna byc w dis_b!
    vd0 = None  # Rzut point na crystal_plane
    ...
    be, bz = get_be_bz(crystal.cell, dislocation.b)
    new_u = love_polar((rd, delta), be=be, bz=bz)
    return new_u
...
def step_newton_raphson(u, f, jacobian, lr=1.0, debug=False, **kwargs):
    ...
    if debug:
        ...
        print("Delta")
        print(delta[89])
    # delta u = -1 Jacobian(phi)^-1 * phi
    # and this function should return a new u value
...
def get_betas(crystal, d, point, rotation_matrix, debug=False, dis_tolerance=DIS_TOLERANCE):
    ...
    """
    # Do not calculate points that are close to the dislocation core
    if np.isclose(np.linalg.norm(d.position-point), 0.0):
...
#         return 0.0
#
#     def make_component_func(i):
#
#         def component_func(*l):
#             l_vec = np.asarray(l)
#             result = func(l_vec)[i]
#             print(f"Inside: {l}, result: {result}")
#             return result
#
...
#     result = np.zeros(n)
#     ranges = []
#     for a, b in zip(limits[0].squeeze(), limits[1].squeeze()):
#         ranges.append((a, b))
#     print(ranges)
#     for i in range(n):
#         f = make_component_func(i)
#         integral, _ = quad_vec(f, ranges)
#         result[i] = integral
#     return result

def calculate_integral_vec(func, limits, n=3, debug=False):
    if np.isclose(np.linalg.norm(limits[0]-limits[1]), 0.0):
        return np.zeros(n)
...
def get_u_dash(x_o, u, crystal, d1, d2, d1_rt, d2_rt, debug=False):

    def one_m_beta(l):
        betas_1 = get_betas(crystal, d1, l.reshape(-1, 3), d1_rt)
        betas_2 = get_betas(crystal, d2, l.reshape(-1, 3), d2_rt)
        betas = betas_1 + betas_2
        one_m_beta = (broadcast_eye(2, betas.shape[0]) - betas)
        ones = broadcast_eye(3, betas.shape[0])
        ones[:, :2, :2] = one_m_beta
        return ones

    return calculate_integral_vec(one_m_beta, (x_o, x_o+u), debug=debug)


# VALUE FUNCTION AND JACOBIAN FOR ITERATING ATOMS, POINTS AND DISLOCATION POSITIONS
def phi_points(crystal, u, x, crystal_plane, d1, d2, debug=True, rotate=False, d1_local=None, d1_rt=None, d2_rt=None):
    current_x = x + u
    current_x = current_x.reshape(-1, 3)
    result = []
    for i, (p, start_p) in enumerate(zip(current_x, x)):
        # print(f"Atom: {i} / {current_x.shape[0]}", end="\r")
        ...
        #     new_u[2] = 0.0
        # else:
        f_tilde = get_f(crystal_plane, point=p, debug=debug, dislocation=d2, crystal=crystal)
        u_dash = get_u_dash(x_o=start_p, u=u[i], crystal=crystal, d1=d1, d2=d2, d1_rt=d1_rt, d2_rt=d2_rt)
        r = u_dash - f_tilde
        result.append(r)

        if debug:
            if i in {89}:
                print(f"Atom {i}: displacement: {r}, f~: {f_tilde}, u^: {u_dash}")
...
def get_cp_integral(l, y_start, d1_local, d2_local, d1_rt, d2_rt, enable_beta2=True):
    ...
    ode_res = scipy.integrate.solve_ivp(func, t_span=(np.squeeze(d1_local.position)[0], -30), y0=[y_start], method="BDF")
...
def find_cp_integral(l, d1_local, d2_local, d1_rt, d2_rt, y0_range=(-1, 1), npoints=100):
    ...
    # FIND THE GLIDE PLANE CLOSEST TO THE D2
    for y0 in y0s:
...
def displace_all(
    ...
    for i in range(1, n_iter+1):
        ...
        # ---------- DETERMINE THE NEW POSITION and ORIENTATION OF d_1 (delta u for d_1)
        def jac(u, x):
        ...
        def u_func(u, x):
            return phi_points(crystal, u, x, crystal_plane=None, d2=d2_local, d1=d1_current_local)
        ...
        ## ---------- DETERMINE THE NEW POSITION OF ATOMS (delta u for atoms)
        # POINTS
        def jac(u, x):
        ...
        def u_func(u, x):
            return phi_points(
                crystal,
                u, x,
                crystal_plane=crystal_plane_current,
                d1=d1_current_local,
                d2=d2_local, rotate=True, debug=True,
                d1_local=d1_current_local, d1_rt=d1_rt_current, d2_rt=d2_rt)
