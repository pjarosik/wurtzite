...
def get_crystal_surface_y0(l0, dislocation, xy, x0=0, n=100000, y0lim=(-30, 30)):
    ...
    def F(x, y, x0, y0, nu, bx, r0):
        return y - y0 + bx / (8 * np.pi * (1 - nu)) * ((1 - 2 * nu) * np.log((x ** 2 + y ** 2) / r0 ** 2) - 2 * y ** 2 / (x ** 2 + y ** 2)- (1 - 2 * nu) * np.log(
...
def love_polar(x: np.ndarray, be: float, bz: float) -> np.ndarray:
    ...
    rr = (1.0-2*NU)*np.log(r**2/r02)
    uy = - be / (8 * np.pi * (1 - NU)) * (rr - 2 * (np.sin(theta) ** 2))
    # uy = 0.0 if uy < 0.0 else uy
