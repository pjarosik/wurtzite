...
def f_points(crystal, u, x, crystal_plane, d2, debug=False, d1_local=None):
    current_x = x + u
    ...
    for i, p in enumerate(current_x):
        ...
        #     new_u[2] = 0.0
        # else:
        r, delta = get_love_compensation(crystal_plane, point=p, debug=debug)
        new_u = love_polar((r, delta), be=be, bz=bz)
        if d1_local is not None:
            r, delta = get_love_compensation(crystal_plane, point=p-np.asarray(d1_local.position), debug=debug)
            new_u_d1 = love_polar((r, delta), be=be, bz=bz)
            d1_distance = np.hypot(p[0]-d1_local.position[0], p[1]-d1_local.position[1])
            d2_distance = np.hypot(p[0], p[1])
            d1_coeff = d1_distance/(d1_distance+d2_distance)
            d2_coeff = d2_distance/(d1_distance+d2_distance)
            # new_u = new_u_d1*d1_coeff + new_u*d2_coeff
            # new_u = new_u_d1
        us.append(new_u)
        if i in {85, 89, 93}:
...
def displace_all(
    ...
    for i in range(1, n_iter+1):
        ...
        def u_func(u, x):
            return f_points(crystal, u, x, crystal_plane=crystal_plane_current, d2=d2_local, d1_local=d1_current_local)
        # ATOMS
        ...
        if points is not None:
            ...
            def u_func_points(u, x):
                return f_points(crystal, u, x, crystal_plane=crystal_plane_current, d2=d2_local, debug=False, d1_local=d1_current_local)
