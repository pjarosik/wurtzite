    ...
    for i in range(1, n_iter+1):
        ...
        for j, x in enumerate(atoms_orig):
            ...
            current_p, new_uuu = convert_to_cp_distance(
                x, u_atoms_current[j], crystal_plane_atoms_orig[j],
                direction=direction# , debug=j==93
            )
            u_atoms_current[j] = new_uuu # np.squeeze(current_p)-np.squeeze(x)
            ...
            if j in {93}:
                ...
                import time
                plt.figure(figsize=(10, 4))
                plt.plot(crystal_plane_current[:, 0], crystal_plane_current[:, 1])
                plt.plot(crystal_plane_atoms_orig[j][:, 0], crystal_plane_atoms_orig[j][:, 1])
                # plt.scatter(initial_atom_position[0], initial_atom_position[1], color="grey", s=75, label="$initial$")
                plt.scatter(x.squeeze()[0] + new_uuu[0], x.squeeze()[1] + new_uuu[1], color="red", s=75, label="$f_y$")
                # plt.scatter(p[0]+new_uuu[0], p[1]+new_uuu[1], color="red", s=75, label="$f_y$")
                # plt.scatter(p[0]+new_u[0], p[1]+u_y_sin, color="green", s=50, label="$f_{y, sin}$")
                # plt.scatter(p[0]+new_u[0], p[1]+u_y_log, color="blue", s=50, label="$f_{y, log}$")
                plt.scatter(0, 0)
                plt.xlim(-20, 0)
                plt.ylim(-3, 3)
                # plt.legend()
                plt.savefig(f"oo_{time.time_ns()}__t.png")
        ...
        # Store
        print("U CRYSTAL PLANE CURRENT")
        print(u_crystal_planes.shape)
